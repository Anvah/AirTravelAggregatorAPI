...
Каждый ответ от API обернут в стандартую форму ответа, где есть тело ответа (если нужно), статус запроса (успешный или нет) и информация об ошибке. Это сделано с целью более удобной обработки на клиенте. В случае если запрашиваемы данные не найдены, клиент в данный момент получает ошибку, а не пустой массив или объект. Это сделано, чтобы данную ситацию в любом случае пришлось дополнительно обработать.
- В дальнейшем, если приложение предполагает развитие в виде микросервисной архитектуры данные объекты (модели стандартных ответов) следует вынести в библиотеку, и подключать через локальный nuget, чтобы сохранить стандартизацию ответов во всей системе.
...
```
Сущнности котырые можно получить в результате запросов:
